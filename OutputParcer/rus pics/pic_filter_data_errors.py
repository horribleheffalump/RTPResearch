...
for i in range(0, len(t)-1):
    if (Prob1[i] + Prob2[i] + Prob3[i] > 0):
        tshort.append(t[i])
        P1.append((E1[i] - Prob1[i]))
        P2.append((E2[i] - Prob2[i]))
        P3.append((E3[i] - Prob3[i]))
...
for i in range(0, len(tExp)-1):
    if (Prob1Exp[i] + Prob2Exp[i] + Prob3Exp[i] > 0):
        tshortExp.append(tExp[i])
        P1Exp.append((E1[i] - Prob1Exp[i]))
        P2Exp.append((E2[i] - Prob2Exp[i]))
        P3Exp.append((E3[i] - Prob3Exp[i]))
...
ax1 = subplot(311)
ax1.plot(tshortExp, P1Exp, color='gray', alpha = 0.7, linewidth=2.0)
...
ax2.plot(tshortExp, P2Exp,color='gray', alpha = 0.7, linewidth=2.0)
...
ax3.plot(tshortExp, P3Exp, color='gray', alpha = 0.7, linewidth=2.0)
...
savefilename = u"D:/Наука/_Статьи/__в работе/2015 - new2an - rus/pic_estimates_errs.pdf";
